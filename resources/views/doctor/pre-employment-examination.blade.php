            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-4">
                    <div class="bg-blue-800 bg-opacity-50 rounded-lg px-4 py-2 border border-blue-500">
                        ...
                        @if($examination->id == 2 && $examination->name == 'Ken Tuazon')
                            <p class="text-green-300 text-xs font-medium">✓ Has X-ray data</p>
                        @endif
                    </div>
                </div>
            </div>
        ...
        <div class="p-8 bg-green-50">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div class="bg-white rounded-lg p-4 border-l-4 border-blue-500 hover:shadow-md transition-shadow duration-200">
                    ...
                    <div class="text-lg font-bold text-blue-800">{{ $examination->preEmploymentRecord->full_name ?? ($examination->preEmploymentRecord->first_name . ' ' . $examination->preEmploymentRecord->last_name) }}</div>
                </div>
                <div class="bg-white rounded-lg p-4 border-l-4 border-green-500 hover:shadow-md transition-shadow duration-200">
                    ...
                    <div class="text-lg font-bold text-green-800">{{ $examination->preEmploymentRecord->age ?? 'N/A' }} years</div>
                </div>
                <div class="bg-white rounded-lg p-4 border-l-4 border-indigo-500 hover:shadow-md transition-shadow duration-200">
                    ...
                    <div class="text-lg font-bold text-indigo-800">{{ $examination->preEmploymentRecord->sex ? ucfirst($examination->preEmploymentRecord->sex) : 'N/A' }}</div>
                </div>
                <div class="bg-white rounded-lg p-4 border-l-4 border-yellow-500 hover:shadow-md transition-shadow duration-200">
                    ...
                    <div class="text-sm font-semibold text-yellow-800 truncate">{{ $examination->preEmploymentRecord->company_name ?? 'N/A' }}</div>
                </div>
            </div>
        </div>
        @endif
        ...
        <div class="p-8">
                                ...
                                <table class="min-w-full bg-white rounded-lg overflow-hidden shadow-sm border border-gray-200">
                                    ...
                                    <tbody class="divide-y divide-gray-200">
                                        ...
                                        <tr class="hover:bg-indigo-50 transition-colors duration-200">
                                            ...
                                            <td class="px-4 py-4 text-sm text-gray-700">
                                                @if($medicalTest && $medicalTest->referenceRanges->count() > 0)
                                                    <div class="space-y-1">
                                                        @foreach($medicalTest->referenceRanges as $range)
                                                            <div class="text-xs">
                                                                <span class="font-medium text-gray-600">{{ $range->reference_name }}:</span>
                                            ...
                                            <td class="px-4 py-4 text-sm text-gray-700">
                                                @if(isset($data['findings']))
                                                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium 
                                                        @if(strtolower($data['findings']) === 'ok' || strtolower($data['findings']) === 'normal')
                                                ...
                                                @else
                                                    @if($testName === 'Chest X-Ray')
                                                        <div class="text-center">
                                                            <span class="text-gray-400 italic text-xs">Lab report not available</span><br>
                                                            <a href="#chest-xray-section" class="text-purple-600 hover:text-purple-800 text-xs font-medium">
                                                                <i class="fas fa-arrow-down mr-1"></i>View Radiologist Results Below
                                                            </a>
                                                        </div>
                                                    @else
                                        ...
                                        @if(isset($labReport['additional_notes']) && $labReport['additional_notes'])
                                        <tr class="bg-gray-50">
                                            <td class="px-4 py-4 text-sm font-semibold text-gray-900">
                                                <div class="flex items-center">
            ...
            @php
                ...
                if($examination->preEmploymentRecord) {
                    ...
                    if (!$xrayChecklist) {
                        $fullName = $examination->preEmploymentRecord->first_name . ' ' . $examination->preEmploymentRecord->last_name;
                ...
                <div class="px-6 py-4 bg-gradient-to-r from-purple-500 to-purple-600">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            ...
                            @if($chestXrayData)
                                <span class="ml-2 text-green-300 text-sm">
                                    ({{ $chestXrayData['result'] ?? 'No result' }})
                                </span>
                            @endif
                        </div>
            ...
            @php
                ...
                $drugTestResults = $examination->drugTestResults()->latest()->first();
            @endphp
            
            <x-drug-test-form 
                exam-type="pre-employment"
                :patient-data="[
                    'name' => $examination->preEmploymentRecord->full_name ?? ($examination->preEmploymentRecord->first_name . ' ' . $examination->preEmploymentRecord->last_name),
                    'address' => $examination->preEmploymentRecord->address ?? '',
                    'age' => $examination->preEmploymentRecord->age ?? '',
                    'gender' => ucfirst($examination->preEmploymentRecord->sex ?? '')
                ]"
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0 sm:space-x-4 mt-8">
                <div class="flex items-center space-x-4">
                    <a href="{{ route('doctor.pre-employment') }}" class="inline-flex items-center px-6 py-3 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200">
                        <i class="fas fa-arrow-left mr-2"></i>Back to List
                    </a>
                </div>
                <div class="flex items-center space-x-4">
                    @if($examination->preEmploymentRecord)
                        <form action="{{ route('doctor.pre-employment.by-record.submit', $examination->preEmploymentRecord->id) }}" method="POST" class="inline">
                            @csrf
                    ...
                    @endif
                    @if($examination->preEmploymentRecord)
                        <a href="{{ route('doctor.pre-employment.edit', $examination->preEmploymentRecord->id) }}" class="inline-flex items-center px-6 py-3 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200">
