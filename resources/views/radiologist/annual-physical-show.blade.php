...
@section('title', 'Annual Physical X-Ray Review')
@section('page-title', 'Annual Physical X-Ray Review')
...
<div class="space-y-6">
        ...
        <div class="px-8 py-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-slate-300">Annual Physical Examination</p>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="bg-gradient-to-r from-slate-50 to-slate-100 px-6 py-4 border-b border-slate-200">
                        <div class="flex items-center justify-between">
                            <h3 class="text-lg font-semibold text-gray-900">
                                <i class="fas fa-image text-slate-600 mr-2"></i>X-Ray Image
                            </h3>
                            <span class="text-xs bg-slate-600 text-white px-3 py-1 rounded-full font-medium">
                                <i class="fas fa-expand-arrows-alt mr-1"></i>Click to Enlarge
                            </span>
                        </div>
                    </div>
                    <div class="p-6">
                        <div class="relative bg-slate-900 rounded-lg overflow-hidden border-2 border-slate-700 cursor-pointer hover:border-slate-500 transition-colors group" id="xray-thumb">
                            ...
                            <div class="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all flex items-center justify-center">
                                <div class="opacity-0 group-hover:opacity-100 transition-opacity">
                                    <div class="bg-white/90 backdrop-blur-sm rounded-full p-4">
                                        <i class="fas fa-search-plus text-slate-700 text-2xl"></i>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <!-- Right Column - Patient Info & Review Form -->
        <div class="space-y-6">
            <!-- Patient Information Card -->
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                <div class="bg-gradient-to-r from-emerald-50 to-emerald-100 px-6 py-4 border-b border-emerald-200">
                    <h3 class="text-lg font-semibold text-gray-900">
                        <i class="fas fa-user text-emerald-600 mr-2"></i>Patient Information
                    </h3>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                <div class="bg-gradient-to-r from-slate-50 to-slate-100 px-6 py-4 border-b border-slate-200">
                    <h3 class="text-lg font-semibold text-gray-900">
                        <i class="fas fa-clipboard-check text-slate-600 mr-2"></i>X-Ray Review
                    </h3>
                </div>
                <form action="{{ route('radiologist.annual-physical.update', request()->route('id')) }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    @method('PATCH')
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-3">
                            <i class="fas fa-x-ray text-slate-600 mr-2"></i>Examination
                        </label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-3">
                            <i class="fas fa-stethoscope text-slate-600 mr-2"></i>Result
                        </label>
                        <select name="cxr_result" 
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-slate-500 focus:border-slate-500 transition-colors text-sm font-medium">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-3">
                            <i class="fas fa-notes-medical text-slate-600 mr-2"></i>Clinical Findings
                        </label>
                        <textarea name="cxr_finding" 
                                  rows="4" 
                                  class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-slate-500 focus:border-slate-500 transition-colors text-sm resize-none"
                    ...
                    <div class="flex items-center justify-between pt-4 border-t border-gray-200">
                        ...
                        <button type="submit" 
                                class="inline-flex items-center px-6 py-2.5 bg-slate-600 hover:bg-slate-700 text-white rounded-lg font-medium transition-colors shadow-sm">
